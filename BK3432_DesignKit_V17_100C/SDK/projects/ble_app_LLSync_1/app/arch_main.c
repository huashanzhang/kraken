...
extern ble_qiot_ret_status_t ble_qiot_explorer_init(void);
...
void bdaddr_env_init(void)
{
	...
	//unsigned char test_addr[6]={0xda,0xf1,0x6b,0x57,0xdd,0xc4};
	...
	//memcpy(&co_default_bdaddr,test_addr,6);
}
...
void rw_main(void)
{
    ...
    if(0==gpio_get_input(0x00))
    {
        Delay_ms(10);
        if(0==gpio_get_input(0x00))
          flash_erase_sector(FLASH_SPACE_TYPE_MAIN, BLE_QIOT_RECORD_FLASH_ADDR/4);
    }
	...
	//	oad_updating_user_section_pro();
		...
		if(test_report_pro)
		{
			if(wait_time>=100)
			{
				ble_event_report_property();//TEST
				ble_event_post(0);//TEST
				test_report_pro = 0;
				wait_time = 0;
			}else
			{
