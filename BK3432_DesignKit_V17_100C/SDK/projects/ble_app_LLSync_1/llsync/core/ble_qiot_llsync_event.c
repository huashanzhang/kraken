...
ble_qiot_ret_status_t ble_event_factory_test(const char *buf,
                                       uint16_t buf_len)
{
    uint8_t send_buf[BLE_QIOT_EVENT_BUF_SIZE] = {0};
	if(buf_len>0)
	{
		send_buf[0] = BLE_QIOT_EVENT_UP_FACTORY_TEST;
		send_buf[1] = 0;
		memcpy(&send_buf[2], buf, buf_len);

		if(0 != ble_send_notify(send_buf, buf_len+2))
		    return BLE_QIOT_RS_ERR;
	}
	return BLE_QIOT_RS_OK;
}
