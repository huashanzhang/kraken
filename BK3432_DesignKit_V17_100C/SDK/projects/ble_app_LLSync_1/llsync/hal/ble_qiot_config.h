...
#define ble_qiot_log_hex(level, hex_name, data, data_len) \
    do { uart_printf(hex_name);uart_printf("\r\n");      \
		for(int i=0;i<data_len;i++) uart_printf("0x%x ",data[i]);\
		 uart_printf("\r\n");                               \
    } while (0)
...
// timestamp
#define BLE_QIOT_LOG_PRINT(...) uart_printf(__VA_ARGS__)

// nrf52832xxAA Flash size is 512KB, nrf52832xxAB Flash size is 512KB, be care of the address!
#define BLE_QIOT_RECORD_FLASH_ADDR     0x27C00  // qiot data storage address
#define BLE_QIOT_RECORD_FLASH_PAGESIZE 512     // flash page size, see chip datasheet

#define BLE_QIOT_OTA_BACKUP_ADDR	0X1C000
...
#define BLE_QIOT_USER_DEVELOPER_VERSION "0.0.5"
...
#define BLE_QIOT_OTA_INFO_FLASH_ADDR (BLE_QIOT_RECORD_FLASH_ADDR + 0x50)
#endif
#define BLE_QIOT_DEVICE_INFO_FLASH_ADDR (BLE_QIOT_RECORD_FLASH_ADDR+0x100)  
...
#define BLE_QIOT_RETRY_TIMEOUT  5    // the max interval between two packages, unit: second
// the time spent for device reboot, the server waiting the device version reported after upgrade. unit: second
#define BLE_QIOT_REBOOT_TIME      120
...
#define BLE_QIOT_OTA_BUF_SIZE (512)
#endif
